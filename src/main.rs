...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 创建一个共享的布尔值，用于控制交易线程的阻塞
    let is_withdrawing = Arc::new(Mutex::new(false));
    // 克隆Arc，以便在提现线程中使用
        ...
        loop {
            ...
            if cutoff <= 0 {
                ...
                if solution.is_some() {
                    ...
                    drop(reader);
                    let withdraw_lock = is_withdrawing_clone.lock().await;
                    // 如果正在提现，则等待提现完成
                    while *withdraw_lock {
    ...
    // 5分钟自动提现
    let _rpc_client = Arc::new(rpc_client.clone());
    let _app_rpc_client = _rpc_client.clone();
            ...
            // 等待 1 小时
            {
                ...
                *withdrawing = true;
                ...
                for miner_reward in miner_rewards.iter_mut() {
                    info!(
                        "claiming miner {} reward: {}",
                        miner_reward.pubkey, miner_reward.balance
                    );
                    ...
                    let mut ixs = Vec::new();
                    let prio_fee_ix =
                        ComputeBudgetInstruction::set_compute_unit_price(prio_fee as u64);
                    ixs.push(prio_fee_ix);
                        ...
                        .await
                    {
                        ...
                        match result {
                            Ok(sig) => {
                                ...
                                let ntxn =
                                    app_database.get_txn_by_sig(sig.to_string()).await.unwrap();
...
async fn handle_socket(
    ...
    if app_state.sockets.contains_key(&who) {
        ...
        app_state.sockets.insert(
            who,
            (who_pubkey, Arc::new(Mutex::new(sender)), who.to_string()),
        );
    }
    drop(app_state);
                        ...
                        if diff >= MIN_DIFF {
                                ...
                                .await
                            {
...

                                {
                                    ...
                                    if let Some((_, existing_diff, existing_hashpower, earned)) =
                                        epoch_hashes.submissions.get(&pubkey)
                                    {
                                        let new_diff = if diff > *existing_diff {
                                            diff
                                        } else {
                                            *existing_diff
                                        };
                                        ...
                                        epoch_hashes
                                            .submissions
                                            .insert(pubkey, (miner.id, new_diff, new_hashpower, 0));
                                    } else {
                                        epoch_hashes
                                            .submissions
                                            .insert(pubkey, (miner.id, diff, hashpower, 0));
                                    }
